...
fn insert_remove_local(c: &mut Criterion) {
    ...
    for i in N_INSERTIONS {
        g.bench_with_input(BenchmarkId::new("concurrent_pool", i), i, |b, &i| {
            b.iter_custom(|iters| {
...
fn insert_remove_single_thread(c: &mut Criterion) {
    ...
    for i in N_INSERTIONS {
        group.bench_with_input(BenchmarkId::new("concurrent_pool", i), i, |b, &i| {
            let pool = concurrent_pool::Pool::with_capacity(i);
            b.iter(|| {
                let v: Vec<_> = (0..i)
        ...
        group.bench_with_input(
            BenchmarkId::new("concurrent_pool_reclaim", i),
            i,
            |b, &i| {
                let mut builder = concurrent_pool::Builder::<usize>::new();
                let pool = builder
                    .capacity(i + 1)
                    .prealloc(i)
                    .enable_auto_reclaim()
